import $  from 'jquery';
export function MeasureRuler(dParam){
    let param={
    ...
    var initRuler = function(){
        ...
        for(var i=0;i<setLen;i++){
            ...
            for(let j=0;j<(param.unitSet-1);j++){
        ...
        $("#"+param.wrapperId).on('touchmove',function(e){
            ...
            let pointerVal=Math.floor((limitLeft-nlf)/(10*param.mult))>0?Math.floor((limitLeft-nlf)/(10*param.mult)):0;
        ...
        $("#"+param.wrapperId+" .sizeNo").each(function(idx,ele){
            if(idx>=rulerLNo && idx<setLen-1){
                let iv=(idx-rulerLNo)*param.minUnit*param.unitSet+param.min;
